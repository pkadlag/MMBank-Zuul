...
public class BankAppController {
	...
	public String fundTransfer(@RequestParam("senderAccountNo") int senderAccountNo,
		...
		restTemplate.postForEntity("http://zuulService/TransactionService/transactions/withdraw", senderTransaction, null);
		Transaction receiverTransaction = new Transaction(receiverAccountNo, amount);
		restTemplate.postForEntity("http://zuulService/TransactionService/transactions", receiverTransaction, null);
		model.addAttribute("message", "Success!");
		return "fundTransfer";
	}
	...
	public ModelAndView getStatement(@RequestParam("offset") int offset, @RequestParam("size") int size) {
		...
		CurrentDataSet currentDataSet = restTemplate.getForObject("http://zuulService/TransactionService/transactions",
